...
## 📊 9. 數位寵物與道具商城系統 (v2.0.0) 核心架構與防禦機制

為了提升門市管理的趣味性與同仁的參與度，系統在 v2.0.0 引入了高度遊戲化的「數位寵物養成與進化系統」，將同仁日常的業績表現、店務任務與寵物的屬性及 M幣 商城進行深度綁定。

### A. 德勤化學四屬性進化分支機制 (Deloitte Chemistry 4-Attribute Evolution)
同仁在完成門市任務或登錄實績時，會獲得經驗值 (XP) 以及 M幣。寵物具有四大基礎屬性：
1. **STR 力量**：主要對應門市「毛利與業績表現」指標（Driver 驅動者）。
2. **CON 體質**：主要對應門市「合規性與體質平續率」指標（Guardian 守護者）。
3. **INT 智力**：主要對應門市「通用店務與行政指標」指標（Integrator 整合者）。
4. **PER 感知**：主要對應門市「社群行銷與評論擴展」指標（Pioneer 先鋒者）。

* **三階段進化鏈**：
  * **Stage 1 (數碼蛋/天賦卵)** ➔ **Stage 2 (萌芽體 / 萌芽獸)** ➔ **Stage 3 (幼生體 / 幼生獸)** ➔ **Stage 4~7 (雛形體/成熟體/完全體/究極體，開始分化)**。
  * 在 Stage 3 ➔ 4 時，系統會自動判定同仁 STR/CON/INT/PER 中哪一個數值最高，進而決定進化為對應屬性的寵物（如最高為 STR 則進化為 Driver 驅動者型態的「小鋼鐵獸」）。
  * 進化冷卻時間：不同 Level 所需冷卻時間不同（如 Level 1➔2 為 3小時，Level 2➔3 為 16小時，Level 3➔4 為 24小時）。

### B. 24 款馬尼特色商城道具與穿戴加成
系統提供 12 款消耗性補給品與 12 款屬性強化裝備。
* **消耗品**（如過期促銷飲料、奧客去去噴霧、神祕進化石）：使用後可立即回復 HP、增加淨化值、或縮短進化倒數。
* **屬性裝備**（如德勤徽章、保溫咖啡杯、馬尼黃金戰甲）：同仁可以在背包中選擇「穿戴」或「卸下」。
  * **屬性累加與防資料損毀設計**：為了防止頻繁修改資料表造成資料累加損壞，試算表中只儲存同仁寵物的「基礎屬性」。穿戴裝備帶來的屬性 Buff，僅在讀取 API `getPetStats` 時，於後端動態累加至「最終屬性(attributes)」並回傳。

### C. 寵物命名系統 (自訂暱稱與品種原名共存)
同仁可以在養成卡片上直接點選寵物名字旁邊的 `🖊` 修改寵物暱稱。
* **UI 呈現**：卡片上會同時呈現**自訂暱稱**（如「馬尼二號」）與**進化原名/種類**（如「Stage 2 萌芽體」）。
* **資料儲存**：在 Google Sheets 模式下，會透過 `renamePet` API 更新 `數位寵物_寵物資料` 表的第二欄（寵物名稱）；在離線模式下也會同步更新至本地。

### D. 隨機天賦卵與孵化加成
同仁首次初始化寵物資料時，會從四種不同天賦的蛋中隨機獲得一顆：
* `egg_driver` ➔ **烈焰數據卵** (STR 力量天賦)
* `egg_guardian` ➔ **岩石數據卵** (CON 體質天賦)
* `egg_pioneer` ➔ **閃電數據卵** (PER 感知天賦)
* `egg_integrator` ➔ **冰霜數據卵** (INT 智力天賦)
* **孵化加成**：在 Stage 1 ➔ 2 孵化時，系統會依據蛋的 ID 給予對應的基礎屬性 **`+5` 點天賦加值**。

### E. 離線防禦 (LocalStorage Fallback) 與向下相容機制
* **向下相容防線**：寵物與背包數據全數儲存在新工作表（`數位寵物_同仁屬性`、`數位寵物_寵物資料`、`數位寵物_背包`）中，100% 不干涉、不破壞原有 Orders、Tasks 等的核心資料表讀寫邏輯。
* **離線 Fallback**：若 Google Sheets 因網路等原因無法連線，或是線上 Apps Script 還沒有部署最新代碼，前端 API 會自動無縫降級為 LocalStorage 本地模擬，在瀏覽器快取中儲存寵物與背包數據。這保證了整個門市系統絕對不會當機，實現 100% 業務穩定。
